...
export default function App() {
  const { matches, groupStandings, knockoutMatches, updateMatchScore, resetSimulation } = useWorldCup();
    ...
    <div className="h-screen flex flex-col bg-gray-50 dark:bg-[#0B0F19] transition-colors duration-300 overflow-hidden text-slate-900 dark:text-slate-100">
        ...
        <div className="w-full max-w-[1920px] mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="relative group">
                <button className="flex items-center gap-1 p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-white/5 text-slate-500 dark:text-slate-400">
                    <Globe size={20} />
                    <span className="uppercase text-xs font-bold">{locale.split('-')[0]}</span>
                </button>
                <div className="absolute right-0 top-full mt-2 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-white/10 overflow-hidden hidden group-hover:block animate-fade-in z-50">
                    <button onClick={() => setLocale('en')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-white/5 flex items-center gap-2">
                        <img src="https://flagcdn.com/w20/us.png" className="w-5" alt="US" /> English
                    </button>
                    <button onClick={() => setLocale('es')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-white/5 flex items-center gap-2">
                        <img src="https://flagcdn.com/w20/es.png" className="w-5" alt="ES" /> Español
                    </button>
                    <button onClick={() => setLocale('pt')} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100 dark:hover:bg-white/5 flex items-center gap-2">
      ...
      <main className="flex-1 w-full max-w-[1920px] mx-auto flex overflow-hidden relative">
        ...
        {activeView === 'groups' ? (
            ...
            <aside className="w-20 hidden md:flex flex-col items-center py-6 gap-2 border-r border-gray-200 dark:border-white/5 bg-white/50 dark:bg-slate-900/30 overflow-y-auto custom-scrollbar h-full shrink-0 z-30">
                ...
                {GROUPS.map(g => (
                    <button
                        ...
                        className={`group relative flex items-center justify-center w-10 h-10 rounded-xl font-display font-bold text-lg transition-all duration-200 border ${
                            activeGroup === g 
          ...
          /* Bracket View */
          <div className="w-full h-full overflow-y-auto p-8 animate-fade-in custom-scrollbar flex flex-col">
              
               {/* --- AD SPACE: BRACKET HEADER --- */}
               <div className="max-w-[1600px] mx-auto w-full mb-6">
                    <AdPlaceholder size="leaderboard" />
               </div>

              <div className="max-w-[1600px] mx-auto w-full flex-1">
                  <div className="glass-panel rounded-2xl p-10 border border-black/5 dark:border-white/5 min-h-[600px] bg-grid-black/[0.02] dark:bg-grid-white/[0.02]">
                      <h2 className="text-4xl font-display font-bold mb-8 text-slate-800 dark:text-white">
                        {t('bracket')}
                      </h2>
                      
                      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                          {knockoutMatches.length > 0 ? knockoutMatches.map(match => (
                              <div key={match.id} className="transform hover:scale-105 transition-transform duration-300">
                                  <MatchCard
                                      match={match}
                                      homeTeam={getTeam(match.homeTeamId)}
                                      awayTeam={getTeam(match.awayTeamId)}
